...
사용자가 타입으로 "card"를 넣었을 때 `useGetRegisteredList` 함수가 반환하는 `Data` 타입은 `PocketInfo`라고 유추할 수 있다. 하지만 `useGetRegisteredList` 함수가 반환하는 `Data` 타입은 `PayMethodType`이기 때문에 사용하는 족에서는 `PocketInfo`일 가능성도 있다. <----뭔소리????
